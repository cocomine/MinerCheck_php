...
/**
 * @return array outpute
 */
function delete($contact){
    $url = $contact['data']['url'];
    $output = array();
    if (preg_match('/([0-9]{1,3}\.){3}[0-9]{1,3}:[0-9]{2,5}/', $url) >= 1) {
        ...
        if ($sqlcon->connect_errno) { //檢查連線
            ...
            #插入資料
            $stmt = $sqlcon->prepare("DELETE FROM Miner WHERE IP = ?");
            $stmt->bind_param("s", $url);
            if (!$stmt->execute()) {
                $output['error'] = 500;
                $output['reason'] = "Url deletion failed";
            } else {
                #成功執行
                $output['success'] = 200;
                $output['reason'] = "Url deleted successfully";
            }
        }
    } else {
        #url格式不符
